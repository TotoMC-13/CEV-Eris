/proc/add_note(target_ckey, notetext, timestamp, adminckey, logged = 1, server, checkrights = 1)
	...
	var/DBQuery/query_find_ckey = dbcon.NewQuery("SELECT ckey, exp FROM player WHERE ckey = '[target_ckey]'")
	...
	var/DBQuery/query_noteadd = dbcon.NewQuery("INSERT INTO notes (ckey, timestamp, notetext, adminckey, server) VALUES ('[target_ckey]', '[timestamp]', '[notetext]', '[admin_sql_ckey]', '[server]')")
...
//		ryzorbot("notify", "addnote=[key_name(usr)]&[target_ckey]","[notetext]") Removed until ryzorbot is fixed
...
/proc/remove_note(note_id)
	...
	var/DBQuery/query_find_note_del = dbcon.NewQuery("SELECT ckey, notetext, adminckey FROM notes WHERE id = [note_id]")
	...
	var/DBQuery/query_del_note = dbcon.NewQuery("DELETE FROM notes WHERE id = [note_id]")
...
/proc/edit_note(note_id)
	...
	var/DBQuery/query_find_note_edit = dbcon.NewQuery("SELECT ckey, notetext, adminckey FROM notes WHERE id = [note_id]")
	...
	if(query_find_note_edit.NextRow())
		...
		var/DBQuery/query_update_note = dbcon.NewQuery("UPDATE notes SET notetext = '[new_note]', last_editor = '[sql_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE id = [note_id]")
	...
	if(target_ckey)
		...
		switch(index)
			...
			if(2)
				...
				search = "^[index]"
		var/DBQuery/query_list_notes = dbcon.NewQuery("SELECT DISTINCT ckey FROM notes WHERE ckey REGEXP '[search]' ORDER BY ckey")
